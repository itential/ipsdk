...
def classify_http_error(
    ...
    if HTTP_INTERNAL_SERVER_ERROR <= status_code < HTTP_STATUS_600:
...
def classify_httpx_error(
    ...
    if isinstance(exc, httpx.TimeoutException):
        return TimeoutError(
            message=f"Request timed out: {exc!s}",
            details={"request_url": request_url, "original_error": str(exc)},
        )

    if isinstance(exc, httpx.ConnectError):
        return NetworkError(
            message=f"Failed to connect: {exc!s}",
            details={"request_url": request_url, "original_error": str(exc)},
        )
    ...
    if isinstance(exc, httpx.HTTPStatusError):
        try:
            ...
            url = request_url
        return classify_http_error(exc.response.status_code, exc.response, url)

    if isinstance(exc, httpx.RequestError):
        return NetworkError(
            message=f"Request error: {exc!s}",
            details={"request_url": request_url, "original_error": str(exc)},
        )
